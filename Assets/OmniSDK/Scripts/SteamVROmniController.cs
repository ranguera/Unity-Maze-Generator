...
namespace Virtuix
{
    public class SteamVROmniController : OmniController
    {
        ...
        {
            Debug.LogError("Add SteamVR SDK, set the viveCameraRigPrefab to [CameraRig] from the SteamVR prefab folder, and uncomment the STEAMVR_OMNI define in SteamVROmniController.");
        }
#else
        ...
        public override void OmniInitialize()
        {
            ...
            {
                Debug.LogError("Prefab needs to be set to the SteamVR [CameraRig] from the SteamVR Prefabs folder");
                return;
            }
            ...
            if (steamVR_TrackedObjects.Length > 0)
            {
                foreach (SteamVR_TrackedObject tObject in steamVR_TrackedObjects)
                {
                    if (tObject.isActiveAndEnabled)
                    {
                        ...
                        {
                            Debug.Log("Calibration Input Received: " + tObject.index.ToString());
                            return true;
                        }
                    }
                }
        ...
        {
            Debug.Log("MotionData: " + motionData);
            if (motionData != null)
            {
                Debug.Log("Calibrating player's forward direction");
                ...
                hasAligned = false;
                Debug.Log("Saving offset value as " + value);
            ...
            if (!hasAligned)
            {
                ...
                if (!hasFullyInitialized && hasAligned)
                {
                    ...
                    if (motionData != null)
                    {
                        ...
                        Debug.Log("Fully Initialized Omni");
                    }
                }
            }
        }
